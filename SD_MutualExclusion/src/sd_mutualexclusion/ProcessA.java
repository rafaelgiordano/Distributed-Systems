...
public class ProcessA extends Thread {
	...
	/**
	 * Cria o socket servidor na porta 3000.
	 */
	 ...
	 * processo (B e C). Caso o parâmetro não seja uma requisição, significa que
			...
			public void run() {
				try {
					...
					if ( !messageType.equals(REQUEST) ) {
						...
						Socket clientSocket = new Socket("localhost", (sender.equals("2")) ? 25002 : 25003);
						...
						String message_id = "A" + timestamp.toString();
						...
						System.out.println("+ Processo A está esperando por permissão para acessar o recurso");

						while ( okFromProcesses != 2 ) {
							/* Espere pelo reply dos outros dois processos. */
						}

						System.out.println("\t- Processo A possui o recurso!");
						okFromProcesses = 0;
						...
						System.out.println("\t- Processo A não possui mais o recurso.");

						/* Agora o processo A deve enviar REPLY para os processos que permanecem na fila. */
		...
		try {
			...
			while ( (line = inFromClient.readLine()) != null ) {
				...
				if ( messageType.equals(REQUEST) ) {
					...
					System.out.println("\t\tProcesso A sobre o recurso : " + resourceState);

					switch ( resourceState ) {
						case NO_WANT:
							/* Apenas envie OK para o remetente. */
							System.out.println("\t\tProcesso A vai mandar OK para " + sender);
							startClientSide("OK@", sender);
							break;
						...
						case WANT:
							...
							if ( requestTimestamp < timestampReceived ) {
								/* Mensagem REQUEST do processo A possui timestamp menor, logo ele tem direito ao recurso. */
								System.out.println("\t\tProcesso A vai mandar WAIT para " + sender + " porque " + requestTimestamp + " < " + timestampReceived);
								startClientSide("WAIT@", sender);

								/* Adiciona o remetente à fila de processos. Depois que o processo A usar o recurso, ele mandará REPLY para todos os processos na fila. */
								...
								System.out.println("\t\tProcesso A vai mandar OK para " + sender + " porque " + requestTimestamp + " >= " + timestampReceived);
								startClientSide("OK@", sender);
							}
							break;
						...
						case ACCESSING_RESOURCE:
							...
							System.out.println("\t\tProcesso A vai mandar WAIT para " + sender);
